...
def jump_search(ordered_list, item):
    ...
    # NOTE: compute block size -> sqrt(n)
    ...
    while i != len(ordered_list) - 1 and ordered_list[i] <= item:
        ...
        # NOTE: apply linear search as search value is greater than or equal to block element
        if i + block_size > len(ordered_list):
            ...
            if j == -1:
                ...
                return
            return i + j
        # NOTE: search value equals block element
        if ordered_list[i + block_size - 1] == item:
            return i + block_size - 1
        # NOTE: search value is less than block element, so compare with next block
